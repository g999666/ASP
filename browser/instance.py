...
def run_browser_instance(config, shutdown_event=None):
    ...
    launch_options = {"headless": headless_mode, "geoip": True}
        ...
        try:
            with Camoufox(**launch_options) as browser:
                ...
                for nav_attempt in range(1, nav_max_retries + 1):
                    try:
                        logger.info(f"正在导航到: {mask_url_for_logging(expected_url)} (尝试 {nav_attempt}/{nav_max_retries}, 超时设置为 90 秒)")
                        ...
                        # 如果导航成功或是非致命状态码，打破重试循环
                        break

                    except (TimeoutError, PlaywrightError) as e:
                        error_type = "超时" if isinstance(e, TimeoutError) else "Playwright 网络错误"
                        error_message = str(e)
                        
                        logger.error(f"导航到 {mask_url_for_logging(expected_url)} 时发生 {error_type} (尝试 {nav_attempt}/{nav_max_retries})")
                        logger.error(f"错误详情: {error_message}")
                        
                        # 在最后一次尝试失败前，进行诊断和退出
                        if nav_attempt < nav_max_retries:
                            logger.info(f"导航失败，将在 {nav_retry_delay} 秒后进行重试...")
                            time.sleep(nav_retry_delay)
                            continue
                        
                        # 如果是最后一次尝试，则执行原有的诊断逻辑并返回
                        if isinstance(e, TimeoutError):
                            ...
                            try:
                                ...
                                logger.error(f"在尝试进行超时诊断时发生额外错误: {diag_e}")
                        else:
                            ...
                            try:
                                ...
                                logger.error(f"在尝试进行网络错误诊断时发生额外错误: {diag_e}")
